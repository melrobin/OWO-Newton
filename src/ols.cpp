...
extern "C" double **FarAllocateMatrixMemory(int,int);
extern void dump_matrix(int ,int ,char *,double **);
extern void dump_vector(int N,char *fname,double *mat);

double ols_order_bases(int start, int end,double **A,double **R, double **C,
 int *o,int *oc,int Noc,double **tempW,int N,int Nh,int M,double *E)
{
	int i,j,k,m,n,ILin,Ibest,mbest,NLin=0,ILLin;
	double EEbest,EE,*b,*c,sum,g,*aa,*w,total_error=0;
...
int ols_linear_solve(valarray<double> &x,matrix &Ap,valarray<double> b,vector<int> & dependencies)
...
double ols(int Nu,matrix &W,matrix &R,matrix &C,int M,const valarray<double> & Et,double *E,matrix &A)
...
//   #endif
   ...
   dgemm_("T","N",&Nu,&M,&Nu,&ONE,&A[0][0],&Nu,&C[0][0],&Nu,&ZERO,&tempW[0][0],&Nu);

   dgemm_("N","N",&Nu,&M,&Nu,&ONE,&A[0][0],&Nu,&tempW[0][0],&Nu,&ZERO,&W[0][0],&Nu);     
 // #ifdef DEBUG        
 // matrix Ap(Nu,Nu);
 // Ap.make_from_array(A,Nu,Nu);
  //outfile << endl;
  //outfile << Ap;
 // #endif  
  MSE = 0.0;
...
int ols4(int N,int Nh,double **W,double **R,double **C,int M,
		 valarray<double> & Et,double *MSE)
{
	double g,**A;
	double **tempW,total_error,*E;
   int i,k,NLin=0,Noc,n,*o,m,*oc;
   int Nu;

   tempW=FarAllocateMatrixMemory(M,N+Nh+1);
   A=FarAllocateMatrixMemory(N+Nh+1,N+Nh+1);
   ...
   printf("%d\t%f\n",1,total_error);
	...
	//Order the input units by starting parameters 1 and Nu
	ols_order_bases(1,Nu,A,R,C,o,oc,Noc,tempW,N,Nh,M,E);
	// Now order the hidden units
	for (n=0;n<Nh;n++)
	   oc[n] = N + 1 + n;

	Nu = N + Nh + 1;
	Noc = Nh;
   //Call to ols_order_bases to order the hidden units by starting at N+1
   //to the new Nu
   *MSE = ols_order_bases(N+1,Nu,A,R,C,o,oc,Noc,tempW,N,Nh,M,E);
